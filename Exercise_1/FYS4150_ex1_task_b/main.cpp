...
double f_solution(double x){
    return 1.0-x*(1-exp(-10*x)) - exp(-10*x);
}
...
*/
    ...
    //vec_f_approx[1] = vec_f_approx[1]*vec_c[1]/vec_a[1];//Special first f
    for(int i = 2;i<n;i++){
        vec_a[i] = vec_a[i] - (vec_b[i-1]*vec_c[i-1])/vec_a[i-1];
        //cout << vec_a[i] << "and" << (vec_b[i-1]*vec_c[i-1])/vec_a[i-1] << endl;
